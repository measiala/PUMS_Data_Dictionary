...
class VarInfo:
    ...
    def add_value(self,val_low: str,val_high: str,desc):
        ...
        if val not in self.valdict.keys():
            ...
            logging.info('Value %s already exists for this variable, skipping.' % val)
            return False
...
class DataDict:
    ...
    def add_var(self,var,vartype,varlen):
        ...
        if var.upper() not in self.vardict.keys():
            newvar = VarInfo(var.upper(),vartype,varlen)
            self.vars.append(newvar)
            self.vardict[var.upper()] = len(self.vars) - 1
            return True
        else:
...
class MinorRecordType:
    ...
    def add_var(self,var):
        ...
        if var.upper() not in self.vars:
            ...
            logging.warning('Variable %s already exists for this Minor Record Type.' % var)
            return False
