...
def test_add_var_name():
    ...
    v1 = dd.vars[dd.vardict['WGT']]
    assert v1.name == 'WGT'
    assert v1.varlen == 5
    assert v1.vartype == 'N'

    """ Verify var added to PUMSDict """
    assert ('WGT' in pl.rts[pl.rtdict['H']].srts[pl.rts[pl.rtdict['H']].srtdict['BV']].vars) \
        == True

    """ Test character variable """
    assert add_var_name('AGS Character 1',dd,pl,'H','BV') == 'AGS'
    assert ('AGS' in dd.vardict.keys()) == True
    assert ('AGS' in pl.rts[pl.rtdict['H']].srts[pl.rts[pl.rtdict['H']].srtdict['BV']].vars) \
        == True

    v2 = dd.vars[dd.vardict['AGS']]
    assert v2.name == 'AGS'
    assert v2.varlen == 1
    assert v2.vartype == 'C'
...
def test_add_var_value():
    ...
    assert ('b' in v2.valdict.keys()) == True
    assert ('B' in v2.valdict.keys()) == False
    assert v2.valdict['b'] == 'N/A'
...
def test_process_line():
    ...
    assert dd.vars[dd.vardict['WGTP']].vartype == 'N'
